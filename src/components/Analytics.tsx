  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div><h1 className="text-2xl font-black tracking-tight text-slate-950 dark:text-white">{t('analytics.title')}</h1><p className="mt-1 text-sm text-slate-500">{t('analytics.subtitle')}</p></div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Card className="p-4"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">{t('analytics.income')}</div><div className="mt-2 text-2xl font-black text-emerald-600">{formatMoney(summary.income, 'VND', locale)}</div></Card>
        <Card className="p-4"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">{t('analytics.expense')}</div><div className="mt-2 text-2xl font-black text-rose-600">{formatMoney(summary.expense, 'VND', locale)}</div></Card>
        <Card className="p-4"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">{t('analytics.net')}</div><div className={`mt-2 text-2xl font-black ${summary.net >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{formatMoney(summary.net, 'VND', locale)}</div></Card>
      </div>

      <Card className="p-4 sm:p-5">
        <h2 className="font-bold text-slate-900 dark:text-white">{t('analytics.cashflow')}</h2>
        <p className="text-xs text-slate-500">{t('analytics.cashflowHint')}</p>
        {trend.length ? (
      ...
      <div className="grid gap-5 xl:grid-cols-[1.1fr_1fr]">
        <Card className="p-4 sm:p-5">
          <h2 className="font-bold text-slate-900 dark:text-white">{t('analytics.expenseMix')}</h2>
          {breakdown.length ? (
        ...
        <Card className="p-4 sm:p-5">
          <h2 className="font-bold text-slate-900 dark:text-white">{t('analytics.topCategories')}</h2>
          <div className="mt-4 space-y-3">
            ...
            {breakdown.map((item, index) => {
              ...
              return (
                <div key={item.id}>
                  <div className="flex justify-between gap-3 text-sm"><span className="font-semibold text-slate-700 dark:text-slate-200">{item.name}</span><span className="text-slate-500">{formatMoney(item.value, 'VND', locale)}</span></div>
