...
export function BatchOcrReview({
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-indigo-200 bg-indigo-50/40 p-3 dark:border-indigo-500/30 dark:bg-indigo-500/5">
      ...
      <div className="grid min-w-0 gap-3 xl:grid-cols-[250px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-3 rounded-xl bg-white p-3 dark:bg-slate-900">
          <div className="grid min-w-0 gap-3 lg:grid-cols-[minmax(220px,.8fr)_minmax(0,1fr)]">
            ...
            <div className="min-w-0">
              <div className="break-all text-sm font-bold text-slate-800 dark:text-slate-100">{files[active.fileIndex]?.name}</div>
              <label className="mt-2 flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-200">
                <input type="checkbox" checked={active.selected} onChange={(e) => patch({ selected: e.target.checked })} />
                {t('batch.include')}
              </label>
              {active.conflict && (
