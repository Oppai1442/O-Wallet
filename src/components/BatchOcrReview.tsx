...
export function BatchOcrReview({
  ...
  return (
    <div className="min-w-0 space-y-3 overflow-visible rounded-2xl border border-blue-200 bg-blue-50/40 p-3 dark:border-blue-500/30 dark:bg-blue-500/5">
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex items-center gap-2 font-bold text-stone-800 dark:text-stone-100"><Images size={18} /> {t('batch.title')}</div>
        <div className="ml-auto text-xs font-semibold text-stone-500">{t('batch.selectedCount', { selected: selectedCount, total: drafts.length })}</div>
      </div>
      ...
      <div className="grid min-w-0 gap-3 xl:grid-cols-[280px_minmax(0,1fr)]">
          ...
          {drafts.map((draft, index) => {
            ...
            return (
              <button
                ...
                className={`grid w-full min-w-0 grid-cols-[52px_minmax(0,1fr)_auto] items-center gap-2 rounded-xl border p-2 text-left transition ${activeRow ? 'border-blue-500 bg-white shadow-sm dark:bg-stone-900' : 'border-stone-200 bg-white/70 hover:border-blue-300 dark:border-stone-700 dark:bg-stone-900/70'}`}
              >
                ...
                <span className="flex flex-col items-end gap-1">
                  ...
                  {!draft.selected && <SkipForward size={14} className="text-stone-400" />}
                </span>
              </button>
            )
          })}
        ...
        <div className="min-w-0 overflow-visible space-y-3 rounded-xl bg-white p-3 dark:bg-stone-900">
          <div className="grid min-w-0 gap-3 md:grid-cols-[minmax(220px,.8fr)_minmax(0,1fr)]">
            <div className="flex min-h-48 items-center justify-center overflow-hidden rounded-xl bg-stone-950/5 p-2 dark:bg-stone-950/40">
              <img src={previewUrls[active.fileIndex]} alt="" className="max-h-72 w-full select-none object-contain" draggable={false} />
            </div>
            <div className="min-w-0 overflow-hidden">
              <div className="break-words text-sm font-bold text-stone-800 dark:text-stone-100">{files[active.fileIndex]?.name}</div>
              <label className="mt-2 flex min-w-0 items-start gap-2 text-sm font-semibold leading-5 text-stone-700 dark:text-stone-200">
          ...
          <div className="grid min-w-0 gap-3 md:grid-cols-3">
            ...
            <div><Label>{t('modal.category')}</Label><CategoryPicker categories={categories} type={active.type} value={active.categoryId} onChange={(value) => patch({ categoryId: value })} /></div>
            <div><Label>{t('modal.account')}</Label><AccountSelect accounts={accounts} catalogues={catalogues} value={active.accountId} onChange={(value) => patch({ accountId: value })} /></div>
          </div>
          ...
          <details className="rounded-xl bg-stone-50 p-2.5 dark:bg-stone-950/40">
            <summary className="cursor-pointer text-xs font-bold text-stone-600 dark:text-stone-300">{t('modal.rawOcr')}</summary>
            <pre className="mt-2 max-h-36 overflow-auto whitespace-pre-wrap break-words text-[11px] text-stone-500">{active.rawText}</pre>
          </details>
        </div>
      </div>
    </div>
