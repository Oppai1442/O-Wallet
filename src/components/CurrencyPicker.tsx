...
export function CurrencyPicker({ value, onChange, locale = 'vi-VN' }: { value: string; onChange: (code: string) => void; locale?: string }) {
  ...
  // TransactionModal already has a CurrencyPicker but historically it was unrestricted.
  // AccountSelect exposes the source account's pockets in the modal DOM. Restrict only
  // inside that modal, leaving Settings/AccountManager pickers fully searchable.
  useEffect(() => {
