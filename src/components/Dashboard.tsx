...
function groupPieData(data: Array<{ id: string; name: string; value: number }>, otherLabel: string, topCount = 5) {
  ...
  return [...head, { id: '__other__', name: otherLabel, value: tail.reduce((sum, item) => sum + item.value, 0) }]
}
...
export function Dashboard() {
  ...
  const pie = useMemo(() => groupPieData(categoryBreakdown(chartTransactions, categories, (category) => categoryPath(category, categories), t('categories.uncategorized'), chartCurrency), t('common.other')), [chartTransactions, categories, t, chartCurrency])
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.7fr_1fr]">
      ...
      <Card className="p-4 sm:p-5"><div className="font-bold text-stone-900 dark:text-white">{t('dashboard.expenseByCategory')} · {chartCurrency}</div>{pie.length ? <><div className="h-52"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={pie} dataKey="value" nameKey="name" innerRadius={50} outerRadius={78} paddingAngle={pie.length > 1 ? 2 : 0} stroke="none" isAnimationActive={false}>{pie.map((item, index) => <Cell key={item.id} fill={PIE_COLORS[index % PIE_COLORS.length]}/>)}</Pie><Tooltip formatter={(value) => formatMoney(Number(value), chartCurrency, locale)}/></PieChart></ResponsiveContainer></div><div className="space-y-2">{pie.map((item, index) => <div className="flex items-center justify-between gap-3 text-sm" key={item.id}><div className="flex min-w-0 items-center gap-2"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: PIE_COLORS[index % PIE_COLORS.length] }}/><span className="truncate text-stone-600 dark:text-stone-300">{item.name}</span></div><span className="font-semibold">{formatMoney(item.value, chartCurrency, locale)}</span></div>)}</div></> : <div className="mt-4"><EmptyState title={t('dashboard.noExpenseTitle')} text={t('dashboard.noExpenseText')}/></div>}</Card>
    </div>
