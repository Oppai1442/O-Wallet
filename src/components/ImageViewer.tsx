      ...
      <div className="flex max-h-[95vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-slate-900" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 dark:border-slate-800">
          ...
          <div className="flex gap-2">
            {url && <a href={url} download={name} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-100 px-3 py-2 text-sm font-bold text-slate-700 transition hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"><Download size={16} /> {t('image.saveDecrypted')}</a>}
        ...
        <div className="overflow-auto bg-slate-100 p-3 text-center dark:bg-slate-950">
          {error ? <div className="p-10 text-rose-500">{error}</div> : url ? <img src={url} alt={name} className="mx-auto max-h-[80vh] max-w-full rounded-lg" /> : <div className="p-10 text-slate-500">{t('image.decrypting')}</div>}
        </div>
      </div>
    </div>
  )
