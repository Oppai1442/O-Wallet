...
export function LedgerAnalytics({ transactions, categories, currency = 'VND', title, subtitle }: { transactions: Transaction[]; categories: Category[]; currency?: string; title?: string; subtitle?: string }) {
  ...
  const missingFxText = t('fx.missingTransactions', { count: convertedSummary.unconverted, currency })
  const nativeHint = t('fx.nativeHint')
  const convertedHint = t('fx.convertedMonthHint', { currency })
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-3">
      ...
      <div className="flex flex-wrap gap-2">
        <Select className="w-40" value={currencyMode} onChange={(e) => setCurrencyMode(e.target.value as CurrencyViewMode)}><option value="native">{t('view.native')}</option><option value="converted">{t('view.convertTo', { currency })}</option></Select>
    ...
    <Card className="p-3 sm:p-4"><div className="flex items-center justify-between gap-3"><Button variant="ghost" className="px-3" onClick={() => setSelectedMonth((value) => shiftMonth(value, -1))} title={t('analytics.previousMonth')}><ChevronLeft size={18}/></Button><div className="min-w-0 text-center"><div className="text-xs font-semibold uppercase tracking-[.14em] text-stone-400">{t('analytics.monthlyAnalysis')}</div><div className="mt-0.5 text-lg font-semibold capitalize text-stone-950 dark:text-white">{monthLabel}</div>{selectedMonth !== currentMonth && <button type="button" className="mt-1 text-xs font-semibold text-blue-600 hover:underline" onClick={() => setSelectedMonth(currentMonth)}>{t('analytics.backCurrent')}</button>}</div><Button variant="ghost" className="px-3" disabled={nextDisabled} onClick={() => setSelectedMonth((value) => shiftMonth(value, 1))} title={t('analytics.nextMonth')}><ChevronRight size={18}/></Button></div></Card>
    ...
    <div className="grid gap-5 xl:grid-cols-2"><PieCard title={t('analytics.incomeMix')} data={incomeBreakdown} emptyTitle={t('analytics.noIncomeTitle')} emptyText={t('analytics.noIncomeText')}/><PieCard title={t('analytics.expenseMix')} data={expenseBreakdown} emptyTitle={t('analytics.noExpenseTitle')} emptyText={t('analytics.noExpenseText')}/></div>
  </div>
}
