...
import { defaultTransactionCurrency, reportingCurrency, transactionCurrencySettingPatch } from '../lib/currencyPreferences'
...
const COPY = {
  vi: {
    ...
    input: 'Nhập liệu & AI', inputHint: 'Voice, OCR và AI',
    ...
    data: 'Dữ liệu & ứng dụng', dataHint: 'Import, export, storage và thông tin app',
    defaults: 'Mặc định', accounts: 'Tài khoản', categories: 'Danh mục', budgets: 'Ngân sách', rules: 'Rules',
    voice: 'Giọng nói', ocr: 'OCR', ai: 'AI',
    ...
    inputTitle: 'Nhập liệu & tự động hóa', inputText: 'Cấu hình các cách nhập giao dịch thường ngày. Voice, OCR và AI được tách thành từng khu riêng.',
    syncTitle: 'Sync & bảo mật', syncText: 'Google Drive, Quick Unlock, hành vi ghi nhớ phiên và các thao tác khóa/chuyển tài khoản.',
    dataTitle: 'Dữ liệu & ứng dụng', dataText: 'Nhập hoặc xuất dữ liệu, kiểm tra dung lượng local và xem thông tin triển khai ứng dụng.',
    budgetTitle: 'Ngân sách / mục tiêu chi tiêu',
    budgetHint: 'Đây là ngưỡng theo dõi theo tháng, không phải hạn mức. O-Wallet vẫn cho phép ghi giao dịch khi vượt ngân sách; Dashboard đổi sang màu cảnh báo từ 80% và đánh dấu vượt từ 100%.',
    transactionCurrency: 'Tiền tệ mặc định khi thêm giao dịch',
    transactionCurrencyHint: 'Giao dịch mới sẽ dùng mã này nếu thiết bị chưa có tiền tệ dùng gần nhất. Sau khi lưu, O-Wallet sẽ nhớ lựa chọn vừa dùng trên thiết bị này.',
    reportingCurrency: 'Tiền tệ quy đổi / báo cáo',
    reportingCurrencyHint: 'Chỉ dùng khi bật chế độ quy đổi để xem Dashboard/Analytics. Số tiền gốc của giao dịch không bị thay đổi.',
  },
  en: {
    ...
    input: 'Input & AI', inputHint: 'Voice, OCR and AI',
    ...
    inputTitle: 'Input & automation', inputText: 'Configure everyday transaction input. Voice, OCR and AI each live in their own focused area.',
    syncTitle: 'Sync & security', syncText: 'Google Drive, Quick Unlock, session remember behavior, locking and account switching.',
    dataTitle: 'Data & app', dataText: 'Import or export data, inspect local storage and view deployment information.',
    budgetTitle: 'Budget / spending target',
    budgetHint: 'This is a monthly tracking threshold, not a hard spending limit. O-Wallet still records transactions above budget; the Dashboard changes to a warning state from 80% and marks the budget exceeded from 100%.',
    transactionCurrency: 'Default transaction currency',
    transactionCurrencyHint: 'New transactions use this when the device has no last-used currency. After saving, O-Wallet remembers the currency you just used on this device.',
    reportingCurrency: 'Conversion / reporting currency',
    reportingCurrencyHint: 'Used only when conversion view is enabled for Dashboard/Analytics. Original transaction amounts are never rewritten.',
  },
} as const
...
export function Settings() {
  ...
  const reportCurrency = reportingCurrency(settings)
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
      ...
      <div className="min-w-0">
        ...
        {section === 'data' && <div className="space-y-5"><SectionHeading title={L.dataTitle} text={L.dataText}/><ExternalImport/><Card className="p-4 sm:p-5"><h2 className="font-bold">{t('settings.export')}</h2><p className="mt-1 text-sm text-stone-500">{t('settings.exportHint')}</p><div className="mt-3 flex flex-wrap gap-2"><Button variant="secondary" onClick={exportJson}><Download size={16}/>JSON</Button><Button variant="secondary" onClick={exportCsv}><Download size={16}/>CSV</Button></div></Card><Card className="p-4 sm:p-5"><h2 className="font-bold">{t('settings.localStorage')}</h2><div className="mt-4 grid grid-cols-3 gap-3"><Metric label={t('settings.records')} value={String(storage.recordCount)}/><Metric label={t('settings.images')} value={String(storage.imageCount)}/><Metric label={t('settings.encryptedImageBytes')} value={bytesToHuman(storage.imageBytes)}/></div></Card><Card className="p-4 text-xs text-stone-500 sm:p-5"><div className="flex items-center gap-2 font-bold text-stone-700 dark:text-stone-200"><ExternalLink size={15}/>{t('settings.deploymentNote')}</div><p className="mt-2">{t('settings.deploymentText')}</p><div className="mt-3 flex flex-wrap gap-3"><a className="font-semibold text-blue-600 hover:underline dark:text-blue-400" href="./privacy.html" target="_blank" rel="noreferrer">{t('settings.privacy')}</a><a className="font-semibold text-blue-600 hover:underline dark:text-blue-400" href="./terms.html" target="_blank" rel="noreferrer">{t('settings.terms')}</a></div></Card></div>}
      </div>
    </div>
  </div>
