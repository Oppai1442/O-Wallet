  ...
  async function runBatchOcr() {
    ...
    try {
      ...
      for (let index = 0; index < files.length; index += 1) {
        ...
        drafts.push({ id: draftId, fileIndex: index, selected: conflict?.level !== 'exact', type: parsedType, amount: amountValue, currency: resolvedCurrency, occurredAt: localOccurredAt, categoryId: resolvedCategoryId, accountId: resolvedAccountId, destinationAccountId: destinationId, merchant: parsed.merchant ?? '', balanceAfter: parsed.balanceAfter !== undefined ? String(parsed.balanceAfter) : '', description: parsed.description ?? '', rawText: parsed.rawText, sourceBBox: { x: 0, y: 0, width: size.width, height: size.height }, sourceWidth: size.width, sourceHeight: size.height, confidence: result.boxes.length ? result.boxes.reduce((sum, box) => sum + box.confidence, 0) / result.boxes.length : undefined, conflict: conflict ? { level: conflict.level, source: existingIds.has(conflict.transaction.id) ? 'existing' : 'batch', transactionId: conflict.transaction.id, occurredAt: conflict.transaction.occurredAt, amount: conflict.transaction.amount, merchant: conflict.transaction.merchant } : undefined })
