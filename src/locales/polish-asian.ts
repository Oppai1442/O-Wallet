...
export const ASIAN_POLISH: Partial<Record<UiLanguage, Pack>> = {
  ja: {
    ...
    'transactions.pageLabel':'{page}/{pages}ページ · 1ページ最大{size}件','transactions.conversionHint':'表示だけを {currency} に換算します。各取引の元の金額と通貨は変更されません。','transactions.missingSnapshot':'{currency} への保存済み為替レートがないため、元の金額を表示しています。','transactions.previous':'前へ'
  },
  'zh-CN': {
    ...
    'transactions.pageLabel':'第 {page}/{pages} 页 · 每页最多 {size} 笔','transactions.conversionHint':'换算只改变显示方式；每笔交易的原始金额和币种都会保留。','transactions.missingSnapshot':'没有保存到 {currency} 的汇率，当前显示原始金额。','transactions.previous':'上一页'
  },
  'zh-TW': {
    ...
    'transactions.pageLabel':'第 {page}/{pages} 頁 · 每頁最多 {size} 筆','transactions.conversionHint':'換算只改變顯示方式；每筆交易原本的金額與幣別都會保留。','transactions.missingSnapshot':'沒有儲存換算為 {currency} 的匯率，目前顯示原始金額。','transactions.previous':'上一頁'
  },
  th: {
    ...
    'transactions.pageLabel':'หน้า {page}/{pages} · สูงสุด {size} รายการต่อหน้า','transactions.conversionHint':'การแปลงสกุลเงินมีผลเฉพาะการแสดงผล จำนวนเงินและสกุลเงินเดิมของแต่ละรายการยังคงอยู่','transactions.missingSnapshot':'ไม่มีอัตราแลกเปลี่ยนที่บันทึกไว้เป็น {currency} จึงแสดงจำนวนเงินเดิม','transactions.previous':'ก่อนหน้า'
  }
}
